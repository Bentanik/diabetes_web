  ...
  rawData.forEach((row, index) => {
    ...
    try {
      ...
      if (parsedStartTime && parsedEndTime) {
        if (parsedStartTime >= parsedEndTime) {
          validationErrors.push(`Giờ bắt đầu phải nhỏ hơn giờ kết thúc`);
        } else {
          const duration = calculateDuration(parsedStartTime, parsedEndTime);
        ...
        validationErrors.push(`Định dạng giờ không hợp lệ (chỉ hỗ trợ 24h)`);
      }
