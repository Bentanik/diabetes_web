...
export const useConsultationDashboard = ({
    ...
    const { data: consultationData, isLoading: isConsultationLoading, refetch } = useGetConsultationsCursor(
        { doctorId },
        {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const consultationsByDate = allConsultations.reduce((acc, consultation) => {
                ...
                acc[date].count += consultation.consultationTemplates.length;
                acc[date].revenue += consultation.consultationTemplates.length * consultationPrice;
                return acc;
            }, {} as Record<string, { date: string; count: number; revenue: number }>);

            const chartData = Object.values(consultationsByDate).map((item) => ({
                date: item.date,
    ...
    }, [consultationData, dateRange, selectedYear, selectedMonth, selectedWeek]);
