...
export const useConsultationSchedule = (
	...
	const [scheduleData, setScheduleData] = useState<ScheduleData>({ timeTemplates: [] });

	const queryParams = useMemo(() => apiParams || { pageSize: 7, fromDate: "", toDate: "" }, [apiParams]);
	...
	useEffect(() => {
		...
		if (consultationData?.pages) {
			const allConsultations = consultationData.pages.flatMap((page: any) => page.data?.items ?? []);
			if (allConsultations.length > 0) {
				const transformed: ScheduleData = {
					timeTemplates: allConsultations.map((item: any) => {
						const date = item.date.split("T")[0];
						const timeslots = item.consultationTemplates.map((template: any) => {
							const [hourStart, minuteStart] = template.startTime.split(":");
							const [hourEnd, minuteEnd] = template.endTime.split(":");
							return {
								start: `${hourStart.padStart(2, "0")}:${minuteStart.padStart(2, "0")}`,
								end: `${hourEnd.padStart(2, "0")}:${minuteEnd.padStart(2, "0")}`,
								id: template.id,
	...
	const hasConsultationData = useMemo(() => {
		...
		const allItems = consultationData.pages.flatMap((page: any) => page.data?.items ?? []);
