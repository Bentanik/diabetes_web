...
import { ArrowUp, Clipboard, RefreshCcw, ThumbsDown, ThumbsUp } from "lucide-react"
import { useGetChatHistoryService, useSendMessageService } from "@/services/chat/services"

const ChatItem = ({ message, index }: { message: API.TChatMessage, index: number }) => {
    ...
    if (isUser) {
        ...
        return (
            <div key={index} className="flex items-start gap-4 w-full justify-end">
...
export default function CheckMain() {
    ...
    const { mutate: sendMessageAPI, isPending: isSending, isError: isSendError, error: sendError } = useSendMessageService()
        ...
        sendMessageAPI({
            ...
            onError: (error: TMeta) => {
                ...
                const errorMessage: API.TChatMessage = {
                    role: "assistant",
                    content: `Xin lỗi, đã có lỗi xảy ra: ${error?.title || "Không thể kết nối với server"}. Vui lòng thử lại sau.`,
                    created_at: new Date().toISOString()
                }
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-200 mb-6">
                    <div className="flex items-center justify-center h-64">
                        <div className="text-center">
                            ...
                            <p className="text-gray-600 text-lg font-medium">Đang tải lịch sử chat...</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-200 mb-6">
                    <div className="flex items-center justify-center h-64">
                        <div className="text-center">
                            ...
                            <p className="text-gray-400 text-sm mt-2">{error?.message || "Không thể tải lịch sử chat"}</p>
                            <Button
                ...
                <div className="flex flex-col gap-6 overflow-y-auto">
                    {allMessages.length === 0 ? (
                        <div className="flex items-center justify-center h-64">
                            <div className="text-center">
                                <div className="w-16 h-16 bg-[#248fca]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <svg className="w-8 h-8 text-[#248fca]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                    </svg>
                                </div>
                                <p className="text-gray-600 text-lg font-medium">Chào mừng đến với AI Assistant</p>
                                <p className="text-gray-400 text-sm mt-2">Nhập tin nhắn bên dưới để bắt đầu trò chuyện</p>
                            </div>
                        </div>
                    ) : (
                        allMessages.map((message, index) => (
                            <ChatItem key={`${message.created_at}-${index}`} message={message} index={index} />
                        ))
                    )}
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-2xl rounded-tl-md px-4 py-3">
                                    <div className="flex items-center gap-2 text-gray-600">
                                        <div className="flex gap-1">
                                            ...
                                            <div className="w-2 h-2 bg-[#248fca] rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></div>
                                            <div className="w-2 h-2 bg-[#248fca] rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div>
                                        </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500">Enter để gửi, Shift+Enter để xuống dòng</span>
                        {/* Error display */}
                        {isSendError && (
