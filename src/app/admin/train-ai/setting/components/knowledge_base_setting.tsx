        ...
        <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                    <label htmlFor={knowledgeBase.id} className="font-medium text-gray-900 cursor-pointer">
                        {knowledgeBase.name}
                    </label>
                    {knowledgeBase.description && (
                        <p className="text-sm text-gray-600 mt-0.5 line-clamp-2">{knowledgeBase.description}</p>
                    )}
                    <div className="flex items-center gap-4 text-sm text-gray-500 mt-1 flex-wrap">
                        <span className="flex items-center gap-1">
                            <FileTextIcon className="w-3 h-3" />
                            {knowledgeBase.document_count.toLocaleString()} tài liệu
                        </span>
                        <span className="flex items-center gap-1">
...
export default function KnowledgeBaseSetting() {
    const [isOpenCreateKnowlegeModal, setIsOpenCreateKnowlegeModal] = useState(false)
                    ...
                    <div className="space-y-3 min-h-[300px]">
                        {isPending ? (
                            ...
                            <div className="text-center py-12 text-gray-500">
                                ...
                                {debouncedSearchTerm ? (
                                    <>
                                        <p className="text-lg mb-2">Không tìm thấy nội dung nào</p>
                                        <p className="text-sm">Thử tìm kiếm với từ khóa khác</p>
                                    </>
                                ) : (
                                    <>
                                        <p className="text-lg mb-2">Chưa có nội dung nào</p>
                                        <p className="text-sm">Hãy thêm nội dung đầu tiên của bạn</p>
                                    </>
                                )}
                            </div>
                        ) : (
                            knowledgeBases.map((knowledgeBase) => (
                                <KnowledgeBaseItem
                                    ...
                                    isSelected={selectedKnowledgeBases.includes(knowledgeBase.id)}
                    ...
                    <div className="space-y-3 max-h-64 overflow-y-auto">
                        {selectedKnowledgeBasesInfo.length === 0 ? (
                            <div className="text-center py-4">
                                ...
                                <p className="text-gray-500 text-sm">Chưa chọn nội dung nào</p>
                            </div>
                        ) : (
                            selectedKnowledgeBasesInfo.map((knowledgeBase) => (
                                <div key={knowledgeBase?.id} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                                    ...
                                    <button
                                        onClick={() => handleKnowledgeBaseToggle(knowledgeBase?.id || '')}
