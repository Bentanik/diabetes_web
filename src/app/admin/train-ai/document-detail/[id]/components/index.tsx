...
import { FileText, Loader2, AlertCircle, Minimize2, Maximize2 } from 'lucide-react';

const DocumentViewPdf = dynamic(() => import('@/app/admin/train-ai/document-detail/[id]/components/document-view-pdf'), {
    ssr: false,
    loading: () => (
        <div className="h-full flex items-center justify-center bg-gray-50">
            <div className="text-center">
                <Loader2 className="w-8 h-8 animate-spin text-[#248fca] mx-auto mb-4" />
                <p className="text-gray-600 text-sm">Đang tải trình xem PDF...</p>
            </div>
        </div>
    )
});
...
const getDocumentData = (id: string) => {
    const mockDocuments = {
        '123': {
            ...
            description: 'Tài liệu nghiên cứu chi tiết về bệnh đái tháo đường type 2, các phương pháp điều trị và phòng ngừa hiện đại trong y học.',
        ...
        default: {
            ...
            description: 'Nghiên cứu về hệ thống kiểm tra kiểu dữ liệu cho JavaScript, phát triển bởi Facebook Inc. Tài liệu kỹ thuật chi tiết.',
    ...
    return mockDocuments[id as keyof typeof mockDocuments] || mockDocuments.default;
};

const DocumentDetailComponent: React.FC<DocumentDetailComponentProps> = ({ id }) => {
    const pdfViewerRef = useRef<{ scrollToHighlight: (highlightId: string) => void }>(null);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            {/* Main Content */}
            <div className={`transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-50 bg-white pt-4 px-3' : ''}`}>
                <div className={`flex gap-6 transition-all duration-300 ${isFullscreen ? 'h-[calc(100vh-2rem)]' : 'h-[calc(100vh-140px)]'}`}>
                    {/* Left Panel - Document Chunks */}
                    <div className={`transition-all duration-300 ${isFullscreen ? 'w-[35%]' : 'w-[45.5%]'} h-full`}>
                        <DocumentChunks
                    ...
                    <div className={`transition-all duration-300 ${isFullscreen ? 'w-[65%]' : 'w-[54.5%]'} h-full bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-lg relative`}
                        ...
                        <div className="p-4 border-b border-gray-100 bg-gray-50/50">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-medium text-gray-900">Xem tài liệu PDF</h3>
                                    </div>
                                </div>
                                    ...
                                    {selectedHighlights.length > 0 && (
                                        <div className="flex items-center gap-2">
                                            <span className="px-3 py-1 bg-[#248fca]/10 text-[#248fca] rounded-full text-xs font-medium border border-[#248fca]/20">
                                                Trang {selectedHighlights[0].position.pageNumber} / {totalPages}
                                            </span>
                                        </div>
                                    )}
                            ...
                            {pdfError ? (
                                <div className="h-full flex items-center justify-center bg-red-50">
                                    <div className="text-center">
                                        ...
                                        <h3 className="text-lg font-medium text-red-900 mb-2">Không thể tải tài liệu PDF</h3>
                                        <p className="text-red-700 text-sm mb-4">Đã xảy ra lỗi khi tải tài liệu PDF.</p>
                                        <button
                            ...
                            {isRefreshing && (
                                <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-10">
                                    <div className="text-center">
                                        ...
                                        <p className="text-gray-600 text-sm">Đang tải lại...</p>
                                    </div>
                                </div>
                            )}
                        </div>
