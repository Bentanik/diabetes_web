...
export default function DocumentChunkList({ id }: DocumentChunkListProps) {
    ...
    const handleSaveChanges = async () => {
        const data = { document_chunk_ids: document_chunk_changes.map((c) => c.id) }
        showBackdrop()
    ...
    const getQualityBadge = (score: string) => {
        ...
        if (numScore >= 0.8)
            return { variant: "default" as const, label: (numScore * 100).toFixed(0) + "%", className: "bg-emerald-500 text-white" }
        if (numScore >= 0.5)
            return { variant: "secondary" as const, label: (numScore * 100).toFixed(0) + "%", className: "bg-amber-500 text-white" }
        return { variant: "destructive" as const, label: (numScore * 100).toFixed(0) + "%", className: "bg-orange-500 text-white" }
    }
                            ...
                            {currentPageChunks.length === 0 ? (
                                ...
                                currentPageChunks.map((chunk) => {
                                    ...
                                    return (
                                        <Card
                                            key={chunk.id}
                                            className={`border-0 bg-white hover:shadow-lg transition-all duration-300 ${isModified ? "ring-2 ring-blue-400" : ""} ${!isActive ? "opacity-60" : ""}`}
                                            style={{
                                        ...
                                        >
                                                ...
                                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                                                    <div className="flex items-center gap-4">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: "#248fca" }}>
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <Badge className={qualityBadge.className + " font-semibold px-3 py-1"}>{qualityBadge.label}</Badge>
                                                        </div>
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            <Switch
                                                                ...
                                                                style={{ backgroundColor: isActive ? "#248fca" : undefined }}
                                                                id={`switch-${chunk.id}`}
                                                            />
                                                            <span className={`text-sm font-semibold px-3 py-1 rounded-full ${isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                                            ...
                                            <CardContent className="pt-0">
                                                <div className="relative">
                                                    <div className="rounded-xl p-6 border-l-4" style={{ backgroundColor: "#f8fafc", borderLeftColor: "#248fca" }}>
                                                        <p className="text-slate-800 leading-relaxed whitespace-pre-wrap break-words text-sm">
                                                            {isExpanded ? chunk.content : chunk.content.substring(0, 300) + "..."}
                                                        </p>
                    ...
                    <div className="xl:w-1/3 w-full">
                        <div className="sticky top-6 space-y-6">
                            ...
                            <Card className="border-0 bg-white" style={{ boxShadow: "rgba(0, 0, 0, 0.16) 0px 1px 4px" }}>
                                <CardHeader className="pb-6">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: "#248fca" }}>
                                ...
                                <Card className="border-0 bg-white" style={{ boxShadow: "rgba(0, 0, 0, 0.16) 0px 1px 4px" }}>
                                    ...
                                    <CardContent className="space-y-4">
                                        <Button
                                            ...
                                            style={{ backgroundColor: "#248fca", boxShadow: "rgba(0, 0, 0, 0.16) 0px 1px 4px" }}
                                        >
                            ...
                            <Card className="border-0 bg-white" style={{ boxShadow: "rgba(0, 0, 0, 0.16) 0px 1px 4px" }}>
                                <CardContent className="p-6">
                                    <div className="flex items-start gap-4">
                                        <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-white" style={{ backgroundColor: "#248fca" }}>
                                        ...
                                        <div>
                                            ...
                                            <ul className="text-sm text-slate-600 space-y-2 leading-relaxed">
                                                <li className="flex items-start gap-2"><span style={{ color: "#248fca" }} className="mt-1">•</span><span>Nội dung chất lượng cao được ưu tiên hiển thị</span></li>
                                                <li className="flex items-start gap-2"><span style={{ color: "#248fca" }} className="mt-1">•</span><span>Sử dụng công tắc để bật/tắt từng đoạn nội dung</span></li>
                                                <li className="flex items-start gap-2"><span style={{ color: "#248fca" }} className="mt-1">•</span><span>Thay đổi được áp dụng trên toàn bộ hệ thống</span></li>
                                            </ul>
                                        </div>
                                    </div>
                                </CardContent>
