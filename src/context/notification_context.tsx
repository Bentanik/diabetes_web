...
interface NotificationContextType {
  ...
  addNotification: (notification: Omit<NotificationItem, "id" | "time" | "createdAt">) => string;
...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  const audioRefs = React.useRef({
    success: typeof window !== "undefined" ? new Audio("/audio/success_notfi.mp3") : null,
    error: typeof window !== "undefined" ? new Audio("/audio/success_notfi.mp3") : null, // Using same sound for now
    warning: typeof window !== "undefined" ? new Audio("/audio/success_notfi.mp3") : null, // Using same sound for now
    info: typeof window !== "undefined" ? new Audio("/audio/success_notfi.mp3") : null, // Using same sound for now
  });
    ...
    if (typeof window !== "undefined" && !isLoaded) {
      try {
        ...
        if (savedNotifications) {
          ...
          const withUpdatedTimes = parsed.map((notif: NotificationItem) => ({
            ...notif,
            time: formatTime(notif.createdAt)
          }));
        ...
        const savedSoundPreference = localStorage.getItem("notification-sound-enabled");
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined" && isLoaded) {
      try {
        localStorage.setItem("app-notifications", JSON.stringify(state.notifications));
      } catch (error) {
  ...
  const playNotificationSound = React.useCallback((type: NotificationType) => {
    if (typeof window !== "undefined" && audioRefs.current[type] && isSoundEnabled) {
      try {
